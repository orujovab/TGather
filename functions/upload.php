...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        if($filetype != "png" && $filetype != "jpg" && $filetype!= "jpeg" && $filetype != "gif"){
           $error[]="Only JPG, JPEG, PNG & GIF formats are allowed.";
        }
        if(exif_imagetype($_FILES["fileUpload"]["tmp_name"])){ // Check if the file is actually an image.
            if(isset($_POST['profile'])){
                $success = 0;
                $filepath = "assets/images/profiles/" . $_SESSION['user_id'] . '.' . $filetype;
                if(move_uploaded_file($_FILES["fileUpload"]["tmp_name"], $filepath)){
                    $sql5 = "INSERT INTO posts (post_caption, post_public, post_time, post_by)
                            VALUES ('" . $row['name'] . " has changed his profile picture.', 'N', NOW(), {$_SESSION['user_id']})";
                    $query5 = mysqli_query($conn, $sql5);
                    $last_id = mysqli_insert_id($conn);
                    $filepath2 = "assets/images/posts/" . $last_id . '.' . $filetype;
                ...
                $filepath = "assets/images/posts/" . $last_id . '.' . $filetype;
                if(move_uploaded_file($_FILES["fileUpload"]["tmp_name"], $filepath)){
